...
|---|---|---|---|---|
| **BCA** | Partner REST API / Trade Portal Catalogue Feed (JSON/CSV) / Headed DOM Capture (`tools/bca_headed_fetch.py`) | Registered BCA Trade Buyer Account (`Catalog:Read`, `Valuation:Read`) | Read-only lot inspection with CAP Clean Price; headed browser only if API unavailable; no CAPTCHA bypass | API: 1 req/s, max concurrency = 1; Headed DOM: max 1 page/min, max 5 pages/run |
| **Auto Trader** | Auto Trader Connect Developer API (REST / OAuth2) / Market Snapshot Feed | Registered Auto Trader Developer / Commercial Partner Account (`Stock:Read`, `Search:Read`) | Live UK Cash Price listings only; strictly no scraping of consumer website or CAPTCHA bypass | Max 5 req/s; adhere to `Retry-After` on HTTP 429 |
...
- **Headed Browser DOM Capture**: When direct APIs are unavailable, a headed browser session (`tools/bca_headed_fetch.py`) operated under user authentication saves DOM pages locally for parsing by `ManualBcaImporter.import_from_html`.
...
- For headed browser capture:
  - **Cadence**: Maximum 1 page per minute (60 seconds delay between pages).
  - **Scope**: Maximum 5 pages per run.
  - **Storage**: Complete page DOM is saved to local disk and parsed locally.
...
- **Cadence**: Pacing enforced before sending each HTTP request or navigating each browser page.
...
2. **Anti-Bot / CAPTCHA Detection**: Response body, browser page, or headers indicate a Cloudflare, Akamai, or CAPTCHA challenge.
...
2. The system falls back to supported import routes:
   - `ManualBcaImporter`: Ingests JSON/CSV catalogue exports or saved HTML DOM files into `AuctionLot` records via `BcaAcquisition`.
   - `ManualAutoTraderImporter`: Ingests JSON/CSV market snapshot feeds into `MarketSnapshot` via `AutoTraderAcquisition`.
   - `tools/bca_headed_fetch.py`: Headed browser capture workflow (max 5 pages, 1 page/min) for offline local DOM parsing.
