...
1. **Permitted Access Mechanisms**:
   ...
   - **Auto Trader**: Auto Trader Connect Developer API (OAuth2 client
     credentials / API key) for registered partners, or the user-assisted
     unauthenticated headed browser capture command
     (`tools/autotrader_headed_fetch.py`), or saved HTML pages / manual market
     snapshot feeds.
...
2. **Headed Browser Capture Workflow (BCA and Auto Trader)**:
   - Data is acquired through a single user-assisted capture run
     (`tools/bca_headed_fetch.py` for BCA; `tools/autotrader_headed_fetch.py`
     for Auto Trader). The user supplies a search name; the command opens a
     visible browser with a fresh session, the user logs in where required
     (BCA) or simply runs a search (Auto Trader requires no login), and the
     command captures up to a configurable movement limit (default 5) at a
     configurable non-zero delay between movements (default 60 seconds).
   - **Capture Storage**: Each run saves a capture directory under
     `data/captures/<source>/<capture_id>/` that is never overwritten: the
     original page data (`pages/`), the valid parsed car records
     (`records.json`), and a skipped-car log (`skipped.json`) with every skip
     reason. Cars are deduplicated by source ID (BCA lot ID / Auto Trader
     listing ID), keeping the latest captured version.
   - **Strict Pacing**: Configurable movement delay; the default is 60 seconds
     (max 1 movement per minute) and a zero delay is rejected.
   - **Bounded Scope**: Maximum 5 result pages or scroll batches per run by
     default. Auto Trader scrolling stops early when two consecutive movements
     produce no new listing IDs.
   - **Local Offline Parsing**: Captured pages are parsed locally by the
     observation layer (`observe_bca_cards` / `observe_autotrader_cards` with
     their validators); no value for condition, identity, mileage, CAP Clean
     Price, Cash Price, or Seller Type is ever invented.
...
5. **Credential Isolation and Revocation**:
   ...
   - Auto Trader credentials are optional: the user-assisted capture workflow
     requires no login and no credential objects. `AutoTraderCredentials`
     exist strictly within the source access boundary for the Connect API path
     only and are masked in string representations to prevent logging or
     leakage.
